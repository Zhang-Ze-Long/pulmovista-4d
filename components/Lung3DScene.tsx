...
const InteractiveMarker: React.FC<{ 
  ...
  return (
    <group position={finalPos}>
      ...
      <pointLight color="#facc15" intensity={15} distance={40} />
    </group>
  );
};
...
const Lung3DScene: React.FC<{ 
  ...
  return (
    <div className="w-full h-full bg-slate-950 relative">
      <div className="absolute top-4 left-4 z-10 flex flex-col gap-1 pointer-events-none bg-slate-900/50 p-2 rounded backdrop-blur-sm border border-slate-700/50">
      ...
      <Canvas gl={{ antialias: true, alpha: true }}>
        <PerspectiveCamera makeDefault position={[130, 100, 100]} up={[0, 0, 1]} />
        <OrbitControls makeDefault target={[0, 0, 0]} enableDamping dampingFactor={0.05} minDistance={50} maxDistance={350} />
        
        <ambientLight intensity={0.7} color="#e0f2fe" />
        <directionalLight position={[30, 30, 30]} intensity={1.0} color="#ffffff" />
        <directionalLight position={[-20, -20, 20]} intensity={0.3} color="#7dd3fc" />
        <pointLight position={[80, 80, 80]} intensity={0.6} color="#ffffff" />
        ...
        <Grid infiniteGrid fadeDistance={250} sectionColor="#475569" cellColor="#1e293b" position={[0, 0, -50]} rotation={[Math.PI/2, 0, 0]} />
      </Canvas>
    </div>
  );
};
